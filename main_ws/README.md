...
task_planner   ←  natural-language commands  →  Gemini Robotics-ER API
     ↓ /motion_task (JSON String)
motion_planner ←  perception (SAM3) + IK planning
     ↓ /target_ee_pose (Pose)
joint_controller ← QP whole-body servo (arm + mobile base)
     ↓ /joint_commands (JointState) + /cmd_vel (Twist)
```
...
the camera image, and publishes a structured action payload to `/motion_task`.
...
ros2 run task_planner task_planner_node \
  ...
  -p motion_task_topic:=/motion_task
```
...
ros2 topic pub /motion_task std_msgs/msg/String \
  '{ "data": "{\"action\": \"grasp\", \"point\": [640.0, 360.0] }" }'
```
...
ros2 run joint_controller qp_servo_piper_node \
  --ros-args --remap /joint_commands:=/joint_states

# Whole-body (Kachaka + Piper)
ros2 run joint_controller qp_servo_node

# Manual EE-pose target (for testing)
ros2 topic pub /target_ee_pose geometry_msgs/Pose \
  "{position: {x: 0.2, y: 0.1, z: 0.2}, orientation: {w: 0.0, x: 0.0, y: 1.0, z: 0.0}}"
```
